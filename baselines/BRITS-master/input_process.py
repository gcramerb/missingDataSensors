...
patient_ids = []
init_path = os.path.abspath('C:\\Users\\gcram\\Documents\\Datasets\\pysio_dataset\\')
for filename in os.listdir(os.path.join(init_path,'set-a')):
    # the patient data in PhysioNet contains 6-digits
    match = re.search('\d{6}', filename)
    if match:
        id_ = match.group()
        patient_ids.append(id_)
outcomes = os.path.join(init_path,'Outcomes.txt')
out = pd.read_csv(outcomes).set_index('RecordID')['In-hospital_death']
...
output = os.path.join(init_path,'json','json')
fs = open(output, 'w')
...
def parse_id(id_):
    data = pd.read_csv(os.path.join(init_path,'set-a',f'{id_}.txt'))

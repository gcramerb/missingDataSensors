...
class MySet(Dataset):
    def __init__(self):
        ...
        init_path = os.path.abspath('C:\\Users\\gcram\\Documents\\Datasets\\pysio_dataset\\json\\')
        filename = os.path.join(init_path,'data.json')
        self.content = open(filename).readlines()
        self.jsonP =   pd.read_json(filename, orient='records')
        with open(filename) as f:
	        self.jsonT = json.load(f)
